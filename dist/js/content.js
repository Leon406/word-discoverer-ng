...
/* harmony export */   handleLexResult: () => (/* binding */ handleLexResult),
/* harmony export */   handleMachineResult: () => (/* binding */ handleMachineResult),
/* harmony export */   handleRelatedResult: () => (/* binding */ handleRelatedResult)
/* harmony export */ });
...
function handleLexResult(doc, options, transform) {
    var searchResult = {
        result: {
            type: 'lex',
            title: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(doc, '.client_def_hd_hd', transform),
        },
    };
    // pronunciation
    if (options.phsym) {
        var $prons = Array.from(doc.querySelectorAll('.client_def_hd_pn_list'));
        if ($prons.length > 0) {
            searchResult.result.phsym = $prons.map(function (el) {
                var pron = '';
                var $audio = el.querySelector('.client_aud_o');
                if ($audio) {
                    pron = (($audio.getAttribute('onclick') || '').match(/https.*\.mp3/) || [''])[0];
                }
                return {
                    lang: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(el, '.client_def_hd_pn'),
                    pron: pron,
                };
            });
            searchResult.audio = searchResult.result.phsym.reduce(function (audio, _a) {
                var lang = _a.lang, pron = _a.pron;
                if (/us|美/i.test(lang)) {
                    audio.us = pron;
                }
    ...
    // definitions
    if (options.cdef) {
        var $container = doc.querySelector('.client_def_container');
        if ($container) {
            var $defs = Array.from($container.querySelectorAll('.client_def_bar'));
            if ($defs.length > 0) {
                searchResult.result.cdef = $defs.map(function (el) { return ({
                    pos: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(el, '.client_def_title_bar', transform),
                    def: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(el, '.client_def_list', transform),
                }); });
            }
        }
    }
    // tense
    if (options.tense) {
        var $infs = Array.from(doc.querySelectorAll('.client_word_change_word'));
        if ($infs.length > 0) {
            searchResult.result.infs = $infs.map(function (el) { return (el.textContent || '').trim(); });
        }
    }
    if (options.sentence > 0) {
        var $sens = doc.querySelectorAll('.client_sentence_list');
        var sentences = [];
        for (var i = 0; i < $sens.length && sentences.length < options.sentence; i++) {
            var el = $sens[i];
            var mp3 = '';
            var $audio = el.querySelector('.client_aud_o');
            if ($audio) {
            ...
            el.querySelectorAll('.client_sen_en_word').forEach(function ($word) {
                $word.outerHTML = (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($word);
            });
            el.querySelectorAll('.client_sen_cn_word').forEach(function ($word) {
                $word.outerHTML = (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($word, transform);
            });
            el.querySelectorAll('.client_sentence_search').forEach(function ($word) {
                $word.outerHTML = "<span class=\"dictBing-SentenceItem_HL\">".concat((0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($word), "</span>");
            });
            sentences.push({
                en: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getInnerHTML)(HOST, el, '.client_sen_en'),
                chs: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getInnerHTML)(HOST, el, {
                    selector: '.client_sen_cn',
                    transform: transform,
                }),
                source: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(el, '.client_sentence_list_link'),
                mp3: mp3,
            });
        }
    ...
    return (0,_index__WEBPACK_IMPORTED_MODULE_0__.handleNoResult)();
}
function handleMachineResult(doc, transform) {
    var mt = (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(doc, '.client_sen_cn', transform);
    if (mt) {
        return {
            result: {
                type: 'machine',
...
function handleRelatedResult(doc, config, transform) {
    var searchResult = {
        result: {
            type: 'related',
            title: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)(doc, '.client_do_you_mean_title_bar', transform),
            defs: [],
        },
    };
    doc.querySelectorAll('.client_do_you_mean_area').forEach(function ($area) {
        var $defsList = $area.querySelectorAll('.client_do_you_mean_list');
        if ($defsList.length > 0) {
            searchResult.result.defs.push({
                title: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($area, '.client_do_you_mean_title', transform),
                meanings: Array.from($defsList).map(function ($list) {
                    var word = (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($list, '.client_do_you_mean_list_word', transform);
                    return {
                        href: "https://cn.bing.com/dict/search?q=".concat(word),
                        word: word,
                        def: (0,_index__WEBPACK_IMPORTED_MODULE_0__.getText)($list, '.client_do_you_mean_list_def', transform),
                    };
                }),
            });
        }
...
function getText(parent) {
    ...
    var child = selector ? parent.querySelector(selector) : parent;
    if (!child) {
        return '';
    }
...
var defaultDOMPurifyConfig = {
    FORBID_TAGS: ['style'],
    FORBID_ATTR: ['style'],
};
function getHTML(parent, _a) {
    var _b = _a === void 0 ? {} : _a, _c = _b.mode, mode = _c === void 0 ? 'innerHTML' : _c, selector = _b.selector, transform = _b.transform, host = _b.host, _d = _b.config, config = _d === void 0 ? defaultDOMPurifyConfig : _d;
    var node = selector ? parent.querySelector(selector) : parent;
    if (!node) {
        return '';
    }
    if (host) {
        var fillLink = function (el) {
            ...
            if (isInternalPage() && el.getAttribute('srcset')) {
                el.setAttribute('srcset', el.getAttribute('srcset').replace(/(,| |^)\/\//g, function (_, head) { return head + 'https://'; }));
            }
        };
    ...
    var fragment = dompurify__WEBPACK_IMPORTED_MODULE_0__.sanitize(node, __assign(__assign({}, config), { RETURN_DOM_FRAGMENT: true }));
    var content = fragment.firstChild ? fragment.firstChild[mode] : '';
    return transform ? transform(content) : content;
}
function getInnerHTML(host, parent, selectorOrConfig) {
    if (selectorOrConfig === void 0) { selectorOrConfig = {}; }
    return getHTML(parent, typeof selectorOrConfig === 'string' ? { selector: selectorOrConfig, host: host, mode: 'innerHTML' } : __assign(__assign({}, selectorOrConfig), { host: host, mode: 'innerHTML' }));
}
function getOuterHTML(host, parent, selectorOrConfig) {
    if (selectorOrConfig === void 0) { selectorOrConfig = {}; }
    return getHTML(parent, typeof selectorOrConfig === 'string' ? { selector: selectorOrConfig, host: host, mode: 'outerHTML' } : __assign(__assign({}, selectorOrConfig), { host: host, mode: 'outerHTML' }));
}
/**
...
function removeChildren(parent, selector) {
    parent.querySelectorAll(selector).forEach(function (el) { return el.remove(); });
}
/**
 * HEX string to normal string
 */
function decodeHEX(text) {
    return text.replace(/\\x([0-9A-Fa-f]{2})/g, function (m, p1) { return String.fromCharCode(parseInt(p1, 16)); });
}
/**
...
var isInternalPage = function () { return false; };
/**
...
/* harmony import */ var _utils_bing__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./utils/bing */ "./src/utils/bing.ts");
  ...
  chrome.runtime.sendMessage(
    ...
    (res) => {
      ...
      if (doc.querySelector('.client_def_hd_hd')) {
        const { audio, result } = (0,_utils_bing__WEBPACK_IMPORTED_MODULE_1__.handleLexResult)(
          doc,
          {
            tense: true,
            phsym: true,
